"""
train_v10.py — JO-VPPM Training Phase 10 (Joint Proactive Evacuation & Multi-Topology)

Chạy:
    python -m src.analytics.train_v10 --topology vietnam
    python -m src.analytics.train_v10 --topology nsfnet
"""
...
logger = logging.getLogger("DGRL-v10")

# ── Hyperparameters ──────────────────────────────────────────
N_ENVS       = 10   # Hoặc 16 nếu Kaggle mạnh
N_STEPS      = 512
BATCH_SIZE   = 256
TOTAL_STEPS  = 2_000_000 
LEARNING_RATE = 3e-4 # Tăng nhẹ để học embedding không gian mới
...
class AdaptivePenaltyCallback(BaseCallback):
    def __init__(self,
                 ...
                 target_rate: float = 0.015,   # Rút xuống 1.5%
...
def make_env(rank: int, seed: int, data_path: str, topology_name: str, lambda_ref: float = -50.0):
    def _init():
        ...
        reward_calc = RewardCalculator(lambda_latency=lambda_ref, evacuation_penalty=-50.0)
...
def train_dgrl(topology_name: str):
    logger.info(f"BẮT ĐẦU HUẤN LUYỆN JO-VPPM V10 TRÊN MẠNG: {topology_name.upper()}")
    ...
    TENSORBOARD  = os.path.join(RES_DIR, 'logs', f'dgrl_v10_tb_{topology_name}')
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        save_path=MODEL_DIR,
        name_prefix=f'dgrl_v10_ckpt_{topology_name}'
    )
    ...
    try:
        model.learn(total_timesteps=TOTAL_STEPS, callback=callbacks,
                    tb_log_name=f"run_v10_{topology_name}")
        
        final_model_path = os.path.join(MODEL_DIR, f'dgrl_{topology_name}.zip')
        model.save(final_model_path)
        logger.info(f"Huấn luyện thành công! Đã lưu: {final_model_path}")
        
        env_path = os.path.join(MODEL_DIR, f'vec_normalize_{topology_name}.pkl')
        ...
        model.save(os.path.join(MODEL_DIR, f'dgrl_v10_interrupted_{topology_name}.zip'))

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="JO-VPPM v10 Trainer")
